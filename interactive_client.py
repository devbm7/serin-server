...
class InteractiveClient:
    ...
    def create_session(self):
        ...
        # Get resume file
        resume_file = self.get_user_choice("Enter resume file path (or press Enter for default): ")
        if not resume_file:
            resume_file = "Resume v3.01.pdf"
        
        # Check if resume file exists
        if not os.path.exists(resume_file):
            print(f"⚠️  Warning: Resume file '{resume_file}' not found")
            use_default = self.get_user_choice("Use default resume? (y/n): ").lower()
            if use_default != 'y':
                return
